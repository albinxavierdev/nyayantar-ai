...
export default function ChatMessages(
  ...
  const { starterQuestions: defaultStarterQuestions } = useClientConfig();
  const starterQuestions = props.starterQuestions || defaultStarterQuestions;
            ...
            >
              {starterQuestions.map((question, i) => (
                <Button
                  ...
                  className="w-full whitespace-normal p-3 h-auto min-h-[60px] rounded-xl text-sm font-medium transition-all bg-background text-muted-foreground hover:bg-muted-foreground hover:text-primary-foreground border-2 border-slate-500/10"
                >
                  {question}
                </Button>
              ))}
